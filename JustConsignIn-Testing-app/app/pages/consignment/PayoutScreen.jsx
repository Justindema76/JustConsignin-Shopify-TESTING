  ...
  return <>
    ...
    <div className="consignment-body consignment-online-layout">
      ...
      {tab === 'outstanding' && <>
        ...
        {viewMode === 'grouped' && <div className="consignment-item-groups">{allConsignorRows.map(({ consignor, sales, due }) => {
          ...
          return <ByConsignorContainer key={consignor.id} consignor={consignor} itemCount={sales.length} itemLabel={`item${sales.length === 1 ? '' : 's'}`} onOpenConsignor={onOpenConsignor} stats={[{ label: 'Available', value: availableCount }, { label: 'Unpaid', value: unpaidCount }, { label: 'Total', value: money(total) }, { label: 'Due', value: money(due) }]}>
            {unpaidCount > 0 && <><div className="consignment-list-labels consignment-sales-columns"><span>Item</span><span>Sale price</span><span>Due</span><span>Source</span><span>Payout</span><span>Action</span></div>{unpaid.map((item) => {
              ...
              return <div className="consignment-list-row consignment-sales-columns" key={item.id}><div className="consignment-list-item-main"><strong>{item.description || item.itemNumber}</strong><small>#{item.itemNumber || '—'} · {item.dateSold || 'Sold'}</small></div><strong className="consignment-money">{money(salePrice)}</strong><strong className="consignment-money">{money(itemDue)}</strong><span><span className="consignment-badge manual">Manual</span></span><span><span className="consignment-badge unpaid">Unpaid</span></span><span><button type="button" className="consignment-list-action" onClick={() => onStartPayout(consignor.id)}>Review &amp; pay</button></span></div>;
            })}</>}
