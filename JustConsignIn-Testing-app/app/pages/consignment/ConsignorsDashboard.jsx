...
export default function ConsignorsDashboard({
  ...
  async function quickMarkSold(item) {
    ...
    try {
      await updateConsignmentItemStatus(item.id, 'Sold', {
        salePrice,
      ...
      <Header
        ...
        action={
          <div className="consignment-header-actions">
            <button type="button" className="consignment-btn" onClick={handleAddItems}>
              <Plus size={17} />
              Add items
            </button>

            <button type="button" className="consignment-btn secondary" onClick={handleEditConsignor}>
              <Pencil size={17} />
              Edit
            </button>
      ...
      <div className="consignment-body">
        ...
        >
          <div className="consignment-profile-column">
            ...
            <div className="consignment-profile-row">
              ...
              <span className="consignment-profile-copy">
                ...
                {consignor.email ? (
                  <a className="consignment-profile-value consignment-profile-link" href={`mailto:${consignor.email}`}>
                    {consignor.email}
                  </a>
                ) : (
                  <span className="consignment-profile-value">—</span>
          ...
          <div className="consignment-profile-column">
            ...
            <div className="consignment-profile-row detail">
              <span className="consignment-profile-copy">
                <span className="consignment-profile-label">Commission split</span>
                <span className="consignment-profile-value">Consignor gets {consignor.commissionPct}%</span>
              </span>
            </div>
            ...
            <div className="consignment-profile-row detail">
              <span className="consignment-profile-copy">
                ...
                <span className="consignment-profile-value">{consignor.dateJoined || '—'}</span>
              </span>
            </div>

            <div className="consignment-profile-row detail">
              <span className="consignment-profile-copy">
                <span className="consignment-profile-label">Unsold items</span>
                <span className="consignment-profile-value">{consignor.unsoldPreference || 'Please return'}</span>
              </span>
            </div>
          </div>
        </section>
        ...
        <div className="consignment-consignor-stats">
          ...
          <div className="consignment-consignor-stat">
            ...
            <strong aria-label="Not available yet">&nbsp;</strong>
          </div>
        </div>
        ...
        {consignorItems.length === 0 && (
          <div className="consignment-empty">
            <h3>No items yet</h3>
            <p>Add the first consignment item for {consignor.firstName}.</p>
            <button type="button" className="consignment-btn" onClick={handleAddItems}>
            ...
            {consignorItems.map((item) => {
              ...
              return (
                <EntityCard
                  ...
                  metrics={
                    sold
                      ? [
                          ...
                          { label: 'Consignor due', value: money(consignorDue) },
                        ]
                      : [
                  ...
                  detailLabel={sold ? 'Sale date' : 'Status'}
                  detailValue={sold ? item.dateSold || 'Sold' : undefined}
                  detailBadge={{
              ...
              return (
                <div className="consignment-dashboard-list-row" key={item.id}>
                  ...
                  <strong className="consignment-dashboard-list-value">
                    {money(sold ? salePrice : item.price)}
                  </strong>

                  <span className="consignment-dashboard-list-value">{commissionPct}%</span>

                  <span>
                    <span className={`consignment-product-badge ${product.className}`}>
                      {product.text}
                    </span>
                  </span>
                  ...
                  <span>
                    <span
                      className={`consignment-badge ${
                        paid ? 'paid' : sold ? 'unpaid' : statusClass(item.status)
                      }`}
                    >
                      {paid ? 'Paid' : sold ? 'Sold · unpaid' : statusLabel(item.status)}
                    </span>
                  </span>
